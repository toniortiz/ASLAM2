...
int Matcher::searchByProjection(FramePtr F, const vector<Landmark*>& vpLandmarks, const double th)
{
    int nmatches = 0;

    const bool bFactor = th != 1.0f;

    for (Landmark* pMP : vpLandmarks) {
        if (!pMP->_trackInView)
            continue;
...
int Matcher::searchByProjection(KeyFrame* pKF, SE3 Tcw, const vector<Landmark*>& vpPoints, vector<Landmark*>& vpMatched, int th)
{
    SE3 Twc = Tcw.inverse();

    // Set of Landmarks already found in the KeyFrame
    set<Landmark*> spAlreadyFound(vpMatched.begin(), vpMatched.end());
    spAlreadyFound.erase(nullptr);

    int nmatches = 0;

    // For each Candidate Landmark Project and Match
    for (Landmark* pMP : vpPoints) {
...
int Matcher::knnMatch(KeyFrame* pKF1, KeyFrame* pKF2, vector<cv::DMatch>& vM21)
{
    ...
    const vector<Landmark*> vpLandmarks2 = pKF2->getLandmarkMatches();

    vector<Landmark*> vpMatches12 = vector<Landmark*>(vpLandmarks1.size(), nullptr);
    ...
    for (size_t i = 0; i < matchesKnn.size(); i++) {
        ...
        if (m1.distance < _NNratio * m2.distance) {
            ...
            Landmark* pMP1 = vpLandmarks1[i1];
            if (!pMP1)
                continue;
            if (pMP1->isBad())
                continue;

            Landmark* pMP2 = vpLandmarks2[i2];
...
int Matcher::knnMatch(KeyFrame* pKF, FramePtr F, vector<Landmark*>& vpLandmarkMatches, vector<cv::DMatch>& vMatches)
{
    vector<vector<cv::DMatch>> matchesKnn;
    set<int> trainIdxs;
    ...
    const vector<Landmark*> vpLandmarksKF1 = pKF->getLandmarkMatches();
    vpLandmarkMatches = vector<Landmark*>(F->_N, nullptr);
    ...
    for (size_t i = 0; i < matchesKnn.size(); i++) {
        ...
        if (m1.distance < _NNratio * m2.distance) {
            ...
            size_t i1 = static_cast<size_t>(m1.queryIdx);
            size_t i2 = static_cast<size_t>(m1.trainIdx);
            Landmark* pMP = vpLandmarksKF1[i1];
...
int Matcher::fuse(KeyFrame* pKF, const vector<Landmark*>& vpLandmarks, const double th)
{
    SE3 Tcw = pKF->getPose();
    ...
    for (Landmark* pMP : vpLandmarks) {
        ...
        if (bestDist <= TH_LOW) {
            Landmark* pMPinKF = pKF->getLandmark(bestIdx);
            if (pMPinKF) {
                if (!pMPinKF->isBad()) {
...
int Matcher::fuse(KeyFrame* pKF, SE3 Tcw, const vector<Landmark*>& vpPoints, double th, vector<Landmark*>& vpReplacePoint)
{
    // Set of Landmarks already found in the KeyFrame
    const set<Landmark*> spAlreadyFound = pKF->getLandmarks();
...
int Matcher::searchBySE3(KeyFrame* pKF1, KeyFrame* pKF2, vector<Landmark*>& vpMatches12, const Mat33& R12, const Vec3& t12, const double th)
{
    // Camera 1 from world
...
int Matcher::searchByProjection(FramePtr prevFrame, FramePtr currFrame, vector<cv::DMatch>& vMatches, const double th)
{
    ...
    for (size_t i = 0; i < prevFrame->_N; i++) {
        Landmark* pMP = prevFrame->_features[i]->_point;
...
int Matcher::knnMatch(const FramePtr F1, FramePtr F2, vector<cv::DMatch>& vMatches)
{
    ...
    for (size_t i = 0; i < matchesKnn.size(); i++) {
        ...
        if (m1.distance < _NNratio * m2.distance) {
            ...
            size_t i1 = static_cast<size_t>(m1.queryIdx);
            size_t i2 = static_cast<size_t>(m1.trainIdx);
            Landmark* pMP = F1->_features[i1]->_point;
