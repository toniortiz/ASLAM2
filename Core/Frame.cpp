...
bool Frame::isInFrustum(Landmark* pMP, double viewingCosLimit)
{
...
void Frame::addLandmark(const size_t& i, Landmark* pMP) { _features[i]->_point = pMP; }
...
cv::Mat Frame::drawMatchedPoints()
{
    ...
    for (size_t i = 0; i < _N; i++) {
        Landmark* pMP = _features[i]->_point;
        if (pMP) {
