...
void KeyFrame::addLandmark(Landmark* pMP, const size_t& idx)
{
...
void KeyFrame::eraseLandmarkMatch(Landmark* pMP)
{
...
void KeyFrame::replaceLandmarkMatch(const size_t& idx, Landmark* pMP)
{
    _landmarks[idx] = pMP;
}

set<Landmark*> KeyFrame::getLandmarks()
{
    unique_lock<mutex> lock(_mutexFeatures);
    set<Landmark*> s;
    for (Landmark* pMP : _landmarks) {
        if (!pMP)
            continue;
        if (!pMP->isBad())
            s.insert(pMP);
...
vector<Landmark*> KeyFrame::getLandmarkMatches()
{
    unique_lock<mutex> lock(_mutexFeatures);
    return _landmarks;
}
