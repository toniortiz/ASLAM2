...
void LocalMap::searchLocalKFs()
{
    ...
    for (size_t i = 0; i < _curFrame->_N; i++) {
        if (_curFrame->_features[i]->_point) {
            LandmarkPtr pMP = _curFrame->_features[i]->_point;
            if (!pMP->isBad()) {
...
void LocalMap::searchLocalPoints()
{
    ...
    for (KeyFrame* pKF : _localKFs) {
        const vector<LandmarkPtr> vpMPs = pKF->getLandmarkMatches();
...
void LocalMap::projectLocalPoints()
{
    ...
    for (LandmarkPtr pMP : _localPoints) {
...
g2o::EdgeSE3ProjectXYZOnlyPose* LocalMap::addMonoEdge(const FeaturePtr kp, LandmarkPtr pMP)
{
...
g2o::EdgeStereoSE3ProjectXYZOnlyPose* LocalMap::addStereoEdge(const FeaturePtr kpUn, LandmarkPtr pMP)
{
