...
public:
    ...
    int searchByProjection(FramePtr F, const std::vector<LandmarkPtr>& vpLandmarks, const double th = 3);
    ...
    int searchByProjection(KeyFrame* pKF, SE3 Tcw, const std::vector<LandmarkPtr>& vpPoints, std::vector<LandmarkPtr>& vpMatched, int th);
    ...
    int knnMatch(KeyFrame* pKF, FramePtr F, std::vector<LandmarkPtr>& vpLandmarkMatches, std::vector<cv::DMatch>& vMatches);

    // Search matches between Landmarks seen in KF1 and KF2 transforming by a SE3 [R12|t12]
    int searchBySE3(KeyFrame* pKF1, KeyFrame* pKF2, std::vector<LandmarkPtr>& vpMatches12, const Mat33& R12, const Vec3& t12, const double th);

    // Project Landmarks into KeyFrame and search for duplicated Landmarks.
    int fuse(KeyFrame* pKF, const std::vector<LandmarkPtr>& vpLandmarks, const double th = 3.0);

    // Project Landmarks into KeyFrame using a given SE3 and search for duplicated Landmarks.
    int fuse(KeyFrame* pKF, SE3 Tcw, const std::vector<LandmarkPtr>& vpPoints, double th, std::vector<LandmarkPtr>& vpReplacePoint);
