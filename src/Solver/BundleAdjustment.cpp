...
void BundleAdjustment::Global(MapPtr pMap, int nIterations, bool* pbStopFlag, const int nLoopKF, const bool bRobust)
{
    ...
    vector<LandmarkPtr> vpMP = pMap->getAllLandmarks();
    GBA(vpKFs, vpMP, nIterations, pbStopFlag, nLoopKF, bRobust);
}

void BundleAdjustment::GBA(const vector<KeyFrame*>& vpKFs, const vector<LandmarkPtr>& vpMPs,
...
void BundleAdjustment::Local(KeyFrame* pKF, bool* pbStopFlag, MapPtr pMap)
{
    ...
    list<LandmarkPtr> lLocalLandmarks;
    for (KeyFrame* pLocalKF : lLocalKeyFrames) {
        vector<LandmarkPtr> vpMPs = pLocalKF->getLandmarkMatches();
    ...
    for (LandmarkPtr pLocalMP : lLocalLandmarks) {
    ...
    vector<LandmarkPtr> vpLandmarkEdgeMono;
    ...
    if (bDoMore) {
        ...
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            ...
            LandmarkPtr pMP = vpLandmarkEdgeMono[i];
        ...
        for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++) {
            ...
            LandmarkPtr pMP = vpLandmarkEdgeStereo[i];
    ...
    vector<pair<KeyFrame*, LandmarkPtr>> vToErase;
    ...
    if (!vToErase.empty()) {
        for (size_t i = 0; i < vToErase.size(); i++) {
            ...
            LandmarkPtr pMPi = vToErase[i].second;
...
int BundleAdjustment::OptimizeSE3(KeyFrame* pKF1, KeyFrame* pKF2, std::vector<LandmarkPtr>& vpMatches1, g2o::Sim3& g2oS12, const double th2)
{
    g2o::SparseOptimizer optimizer;
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        LandmarkPtr pMP1 = vpLandmarks1[i];
        LandmarkPtr pMP2 = vpMatches1[i];
