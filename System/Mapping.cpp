...
void Mapping::processNewKeyFrame()
{
    ...
    const vector<Landmark*> vpLandmarkMatches = _curKF->getLandmarkMatches();

    for (size_t i = 0; i < vpLandmarkMatches.size(); i++) {
        Landmark* pMP = vpLandmarkMatches[i];
        if (pMP) {
            if (!pMP->isBad()) {
...
{
    list<Landmark*>::iterator lit = _recentAddedLandmarks.begin();
...
void Mapping::searchInNeighbors()
{
    ...
    Matcher matcher;
    vector<Landmark*> vpLandmarkMatches = _curKF->getLandmarkMatches();
    ...
    for (KeyFrame* pKFi : vpTargetKFs) {
        vector<Landmark*> vpLandmarksKFi = pKFi->getLandmarkMatches();

        for (Landmark* pMP : vpLandmarksKFi) {
            if (!pMP)
                continue;
    ...
    for (Landmark* pMP : vpLandmarkMatches) {
        if (pMP) {
            if (!pMP->isBad()) {
                pMP->computeDistinctiveDescriptors();
                pMP->updateNormalAndDepth();
...
void Mapping::KeyFrameCulling()
{
    ...
    for (KeyFrame* pKF : vpLocalKeyFrames) {
        if (pKF->_id == 0)
            continue;
        const vector<Landmark*> vpLandmarks = pKF->getLandmarkMatches();

        const int thObs = 3;
        int nRedundantObservations = 0;
        int nMPs = 0;
        for (size_t i = 0, iend = vpLandmarks.size(); i < iend; i++) {
            Landmark* pMP = vpLandmarks[i];
            if (!pMP)
                continue;
            if (pMP->isBad())
                continue;
