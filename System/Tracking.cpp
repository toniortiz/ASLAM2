...
void Tracking::initialization()
{
    ...
    for (size_t i = 0; i < _curFrame->_N; i++) {
        if (_curFrame->_features[i]->isValid()) {
            ...
            Landmark* pNewMP = new Landmark(xw, pKFini, _map);
...
void Tracking::checkReplacedInLastFrame()
{
    for (size_t i = 0; i < _prevFrame->_N; i++) {
        Landmark* pMP = _prevFrame->_features[i]->_point;

        if (pMP) {
            Landmark* pRep = pMP->getReplaced();
            if (pRep)
...
bool Tracking::trackReferenceKeyFrame()
{
    ...
    vector<Landmark*> vpLandmarkMatches;
...
void Tracking::createNewLandmarks(FramePtr frame, KeyFrame* pKF)
{
    ...
    vDepthIdx.reserve(frame->_N);
    for (size_t i = 0; i < frame->_N; i++) {
        double z = frame->_features[i]->getDepth();
        if (z > 0)
    ...
    for (size_t j = 0; j < vDepthIdx.size(); j++) {
        ...
        Landmark* pMP = frame->_features[i]->_point;
        if (!pMP)
            bCreateNew = true;
        ...
        if (bCreateNew) {
            ...
            if (pKF) {
                ...
                pNewMP = new Landmark(xw, _map, frame.get(), i);
                _tempPoints.push_back(pNewMP);
            }
        ...
        if (vDepthIdx[j].first > _camera->maxDepthTh() && nPoints > 100)
            break;
    }
}
