...
bool LoopClosing::computeSE3()
{
    ...
    for (size_t i = 0; i < nInitialCandidates; i++) {
        ...
        if (!bOK) {
            ...
            vector<Landmark*> vpLandmarkMatches(_curKF->_N, nullptr);
    ...
    for (KeyFrame* pKF : vpLoopConnectedKFs) {
        vector<Landmark*> vpLandmarks = pKF->getLandmarkMatches();
    ...
    int nTotalMatches = 0;
    for (Landmark* pMP : _curMatchedPoints) {
        if (pMP)
            nTotalMatches++;
    }
...
void LoopClosing::correctLoop()
{
...

    {
        ...
        for (auto& [pKFi, g2oCorrectedTiw] : CorrectedSE3) {
            ...
            vector<Landmark*> vpMPsi = pKFi->getLandmarkMatches();
            for (Landmark* pMPi : vpMPsi) {
                if (!pMPi)
                    continue;
                if (pMPi->isBad())
                    continue;
        ...
        for (size_t i = 0; i < _curMatchedPoints.size(); i++) {
            if (_curMatchedPoints[i]) {
                Landmark* pLoopMP = _curMatchedPoints[i];
                Landmark* pCurMP = _curKF->getLandmark(i);
                if (pCurMP)
...
void LoopClosing::searchAndFuse(const KeyFrameAndPose& CorrectedPosesMap)
{
    ...
    for (auto& [pKF, g2oTcw] : CorrectedPosesMap) {
        ...
        vector<Landmark*> vpReplacePoints(_loopLandmarks.size(), nullptr);
        ...
        for (size_t i = 0; i < nLP; i++) {
            Landmark* pRep = vpReplacePoints[i];
            if (pRep) {
...
void LoopClosing::runGlobalBundleAdjustment(int nLoopKF)
{
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if (!_stopGBA) {
            ...
            // Correct Landmarks
            const vector<Landmark*> vpMPs = _map->getAllLandmarks();
